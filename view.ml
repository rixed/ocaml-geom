	...
	let handle_event () =
		...
		(match ev with
			| Some Glop.Resize (w, h) ->
				Cnt.with_mutex new_size_mutex (fun x -> new_size := x) (Some (w, h))
			| _ -> ()) ;
		Cnt.may ev on_event in
	let event_thread () =
		Cnt.forever handle_event () in
	let next_frame () =
		Cnt.with_mutex new_size_mutex (fun () ->
			match !new_size with
				| Some (w, h) ->
					Glop.set_projection_to_winsize z_near z_far w h ;
					new_size := None
				| _ -> ()) () ;
		List.iter (Cnt.apply ()) painters ;
